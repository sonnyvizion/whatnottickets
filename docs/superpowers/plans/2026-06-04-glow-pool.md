# Glow Pool Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Ajouter 6 lueurs rouges/bleues flottantes dans le canvas WebGL existant, qui dérivent doucement et pulsent en opacité.

**Architecture:** Nouveau composant `GlowPool` ajouté dans `CameraFlashes.tsx`, monté dans le `<Canvas>` avant `<FlashPool />`. Même pattern impératif `useEffect` + `useFrame` que `FlashPool`. Les glows sont permanents (jamais invisibles), dérivent avec rebond sur les bords, et pulsent via `sin()`.

**Tech Stack:** three.js, @react-three/fiber, GLSL custom shader (uOpacity + uColor uniforms)

---

## Fichiers

- **Modifier:** `src/components/CameraFlashes.tsx` uniquement

---

## Task 1 : Ajouter GlowPool dans CameraFlashes.tsx

**Files:**
- Modify: `src/components/CameraFlashes.tsx`

- [ ] **Step 1 : Remplacer le contenu de `src/components/CameraFlashes.tsx` par le fichier complet suivant**
...
**Note importante :** Le `fragmentShader` original est renommé `flashFragmentShader`. Le nouveau `glowFragmentShader` utilise `pow(max(0.0, 1.0 - dist), 3.0)` pour un falloff exponentiel plus doux, et reçoit `uColor` (vec3) en uniform. `FlashPool` est mis à jour pour référencer `flashFragmentShader` au lieu de `fragmentShader`.
...
```bash
cd /Users/inspee/Documents/S/Whatnottickets/whatnottickets
npx eslint src/components/CameraFlashes.tsx
```

Expected : aucune erreur. Si `react-hooks/purity` ou `react-hooks/refs` remonté, vérifier qu'aucun `Math.random()` ni accès `ref.current` n'a été introduit dans le corps du composant (hors `useEffect`/`useFrame`).
...
Expected : build réussi, 9/9 pages générées, aucune erreur SSR.

- [ ] **Step 5 : Commit**

```bash
git add src/components/CameraFlashes.tsx
git commit -m "feat: add drifting red/blue glow pool to WebGL background"
```
