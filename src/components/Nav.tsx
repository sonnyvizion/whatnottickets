...
export default function Nav({ ctaLink, ctaLabel }: { ctaLink?: string; ctaLabel?: string }) {
  const [open, setOpen] = useState(false);
  const [hidden, setHidden] = useState(false);
  const lastY = useRef(0);

  const ctaHref = ctaLink || "/#contact";
  const ctaText = ctaLabel || "Obtenir mes billets";
  ...
  const CtaButton = ({ className, onClick }: { className: string; onClick?: () => void }) => {
    const inner = (
      <>
        {IG_SVG}
        <span style={{ lineHeight: 1 }}>{ctaText}</span>
      </>
    );
    return ctaExternal ? (
      <a href={ctaHref} target="_blank" rel="noopener noreferrer" className={className} onClick={onClick}>
        {inner}
      </a>
    ) : (
      <Link href={ctaHref} className={className} onClick={onClick}>
        {inner}
      </Link>
    );
  };
  ...
  return (
    ...
    >
      ...
      <div className={`nav-mobile ${open ? "open" : ""}`}>
        ...
        <CtaButton className="btn-insta nav-mobile-insta" onClick={() => setOpen(false)} />
      </div>
    </header>
  );
}
